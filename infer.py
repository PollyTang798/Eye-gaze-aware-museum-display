...
# This method will show image in any image viewer
                ...
                # with open('eye_gaze.csv', 'w') as f:
                #     # create the csv writer
                #     # print(y_vec[0][0])
                #     writer = csv.writer(f)

                #     # write a row to the csv file
                #     writer.writerows(map(lambda x: [x], y_vec[0][0].item()))
                #     writer.writerow(y_vec[0][0].item())
                #     writer.writerow(y_vec[0][1].item())
                a = np.asarray(y_vec)
                np.savetxt("eye_gaze.csv", a, delimiter=",")
